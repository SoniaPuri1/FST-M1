...
public class Chrome3_LoginPage {
	...
	void activity1() throws InterruptedException {
		...
		try {
			...
			wait.until(ExpectedConditions
					.presenceOfElementLocated(MobileBy.xpath("//android.widget.Button[@text='Login']")));
			try {
			...
			driver.findElement(MobileBy
					.AndroidUIAutomator("UiScrollable(UiSelector().scrollable(true)).scrollIntoView(text(\"Popups\"))"))
					.click();

			// Invalid Credentials
			wait.until(ExpectedConditions
					.presenceOfElementLocated(MobileBy.xpath("//android.widget.Button[@text='Sign In']"))).click();
			// User name & password
